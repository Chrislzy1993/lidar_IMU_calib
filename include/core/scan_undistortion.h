...
public:
  ...
  explicit ScanUndistortion(TrajectoryManager::Ptr traj_manager, std::shared_ptr<IO::LioDataset> dataset)
    : traj_manager_(std::move(traj_manager)), dataset_reader_(std::move(dataset))
  {
  }
  ...
  void undistortScan(bool correct_position = false)
  {
    ...
    for (const TPointCloud& scan_raw : dataset_reader_->get_scan_data())
    {
      ...
      undistort(q_L0_to_G.conjugate(), p_L0_in_G, scan_raw, scan_in_target, correct_position);
  ...
  void undistortScanInMap(const Eigen::aligned_vector<LiDAROdometry::OdomData>& odom_data)
  {
    ...
    for (size_t idx = 0; idx < dataset_reader_->get_scan_data().size(); idx++)
    {
      ...
      // pcl::transformPointCloud(*(iter->second), *scan_inMap, pose);
...
private:
  void undistort(const Eigen::Quaterniond& q_G_to_target, const Eigen::Vector3d& p_target_in_G,
                 const TPointCloud& scan_raw, const VPointCloud::Ptr& scan_in_target,
