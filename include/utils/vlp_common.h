...
public:
  ...
  void unpack_scan(const velodyne_msgs::VelodyneScan::ConstPtr& lidarMsg, TPointCloud& outPointCloud) const
  {
    outPointCloud.clear();
      ...
      for (int block = 0; block < BLOCKS_PER_PACKET; block++, block_counter++)
      {
        ...
        for (int firing = 0, k = 0; firing < FIRINGS_PER_BLOCK; firing++)
        {
          for (int dsr = 0; dsr < SCANS_PER_FIRING; dsr++, k += RAW_SCAN_SIZE)
          {
            ...
            azimuth_corrected_f =
                azimuth + (azimuth_diff * ((dsr * DSR_TOFFSET) + (firing * FIRING_TOFFSET)) / BLOCK_TDURATION);
            azimuth_corrected = ((int)round(azimuth_corrected_f)) % 36000;

            /*condition added to avoid calculating points which are not
          in the interesting defined area (min_angle < area < max_angle)*/
            if ((azimuth_corrected >= m_config.min_angle && azimuth_corrected <= m_config.max_angle &&
                 m_config.min_angle < m_config.max_angle) ||
                (m_config.min_angle > m_config.max_angle &&
                 (azimuth_corrected <= m_config.max_angle || azimuth_corrected >= m_config.min_angle)))
            {
  ...
  void unpack_scan(const sensor_msgs::PointCloud2::ConstPtr& lidarMsg, TPointCloud& outPointCloud) const
  {
    OPointCloud temp_pc;
    pcl::fromROSMsg(*lidarMsg, temp_pc);
    ...
    for (int h = 0; h < temp_pc.height; h++)
    {
      for (int w = 0; w < temp_pc.width; w++)
      {
        outPointCloud.at(w, h) = nan_point;
      }
    }

    int w = 0, h = 0;
    for (size_t i = 0; i < temp_pc.size(); ++i)
    {
      OPoint& in_point = temp_pc.points[i];
      h = in_point.ring;
      w = static_cast<int>(in_point.ori / 0.2);
      if (w < 0 || w > outPointCloud.width - 1)
      {
        std::cout << "ori: " << in_point.ori << std::endl;
        continue;
      }
      TPoint& out_point = outPointCloud.at(w, h);
      out_point.x = in_point.x;
      out_point.y = in_point.x;
      out_point.z = in_point.x;
      out_point.intensity = in_point.intensity;
      out_point.timestamp = in_point.time_stamp;
    }
  }
...
private:
  void setParameters(ModelType modelType)
  {
    ...
    if (modelType == VLP_16)
    {
      ...
      float vert_correction[16] = { -0.2617993877991494,  0.017453292519943295, -0.22689280275926285,
