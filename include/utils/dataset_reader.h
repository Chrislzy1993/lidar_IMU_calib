...
public:
  ...
  void init()
  {
    switch (lidar_model_)
    {
      case LidarModelType::VLP_16:
        p_LidarConvert_ = VelodyneCorrection::Ptr(new VelodyneCorrection(VelodyneCorrection::ModelType::VLP_16));
        break;
  ...
  bool read(const std::string path, const std::string imu_topic, const std::string lidar_topic,
            const double bag_start = -1.0, const double bag_durr = -1.0)
  {
    ...
    for (rosbag::MessageInstance const m : view)
    {
      ...
      if (imu_topic == topic)
      {
        ...
        data_->imu_data_.back().accel = Eigen::Vector3d(imu_msg->linear_acceleration.x, imu_msg->linear_acceleration.y,
        ...
        //         imu_msg->linear_acceleration.z);
        // data_->imu_data_.back().gyro = Eigen::Vector3d(
        //         imu_msg->angular_velocity.y, -imu_msg->angular_velocity.x,
  ...
  void adjustDataset()
  {
    ...
    assert(scan_timestamps.front() < imu_data.back().timestamp && scan_timestamps.back() > imu_data.front().timestamp &&
