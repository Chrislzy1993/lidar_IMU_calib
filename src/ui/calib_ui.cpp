...
void CalibInterface::initGui()
{
  ...
  s_cam_.SetProjectionMatrix(pangolin::ProjectionMatrix(1600, 1000, 2000, 2000, 800, 500, 0.1, 1000));
  ...
  pangolin::Var<std::function<void(void)>> initialization("ui.Initialization",
                                                          std::bind(&CalibInterface::Initialization, this));

  pangolin::Var<std::function<void(void)>> data_association("ui.DataAssociation",
                                                            std::bind(&CalibInterface::DataAssociation, this));

  pangolin::Var<std::function<void(void)>> batch_optimization("ui.BatchOptimization",
                                                              std::bind(&CalibInterface::BatchOptimization, this));

  pangolin::Var<std::function<void(void)>> refinement("ui.Refinement", std::bind(&CalibInterface::Refinement, this));

  pangolin::Var<std::function<void(void)>> save_map("ui.SaveMap", std::bind(&CalibInterface::saveMap, this));

  /// short-cut
  pangolin::RegisterKeyPressCallback('r', [this]() { resetModelView(); });
