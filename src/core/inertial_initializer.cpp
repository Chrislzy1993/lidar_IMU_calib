...
namespace licalib
{
bool InertialInitializer::EstimateRotation(TrajectoryManager::Ptr traj_manager,
                                           const Eigen::aligned_vector<LiDAROdometry::OdomData>& odom_data)
{
  ...
  {
    // compute delta odom in imu sensor
    size_t i = j - 1;
    ...
    // compute delta odom in lidar sensor
    ...
    // compute weight by rotation angle diff
  ...
  // compute result by svd decompose
