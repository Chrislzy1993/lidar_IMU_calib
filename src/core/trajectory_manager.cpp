...
{
  assert(imu_data_.size() > 0 && "[initialSO3TrajWithGyro]: There's NO imu data for initialization.");
...
void TrajectoryManager::trajInitFromSurfel(SurfelAssociation::Ptr surfels_association, bool opt_time_offset_)
{
...
bool TrajectoryManager::evaluateIMUPose(double imu_time, int flags, Result& result) const
{
...
bool TrajectoryManager::evaluateIMUAtt(double imu_time, Eigen::Quaterniond& result) const
{
  if (traj_->MinTime() > imu_time || traj_->MaxTime() <= imu_time)
  {
    std::cout << "out of time range!" << std::endl;
    return false;
  }
  result = traj_->Evaluate(imu_time, EvalOrientation)->orientation;
  return true;
}

bool TrajectoryManager::evaluateLidarPose(double lidar_time, Eigen::Quaterniond& q_LtoG, Eigen::Vector3d& p_LinG) const
{
...
bool TrajectoryManager::evaluateLidarRelativeRotation(double lidar_time1, double lidar_time2,
                                                      Eigen::Quaterniond& q_L2toL1) const
{
  assert(lidar_time1 <= lidar_time2 && "[evaluateRelativeRotation] : lidar_time1 > lidar_time2");
  ...
  // std::cout << "result1: " << result1->orientation.toRotationMatrix().eulerAngles(0, 1, 2).transpose() << std::endl;
  // std::cout << "result2: " << result2->orientation.toRotationMatrix().eulerAngles(0, 1, 2).transpose() << std::endl;
  // std::cout << "calib: " << calib_param_manager->q_LtoI.toRotationMatrix().eulerAngles(0, 1, 2).transpose() <<
  // std::endl;
  // std::cout << "q_L2toL1: " << q_L2toL1.toRotationMatrix().eulerAngles(0, 1, 2).transpose() << std::endl;
...
template <typename TrajectoryModel>
void TrajectoryManager::addSurfMeasurement(std::shared_ptr<kontiki::TrajectoryEstimator<TrajectoryModel>> estimator,
                                           const SurfelAssociation::Ptr surfel_association)
{
  ...
  for (auto const& spoint : surfel_association->get_surfel_points())
  {
    ...
    auto msp = std::make_shared<SurfMeasurement>(lidar_, spoint.point, closest_point_vec_.at(plane_id).data(), time,
                                                 map_time_, 5.0, weight);
...
template <typename TrajectoryModel>
void TrajectoryManager::addCallback(std::shared_ptr<kontiki::TrajectoryEstimator<TrajectoryModel>> estimator)
{
...
void TrajectoryManager::printErrorStatistics(const std::string& intro, bool show_gyro, bool show_accel,
                                             bool show_lidar) const
{
  std::cout << "\n============== " << intro << " ================" << std::endl;
