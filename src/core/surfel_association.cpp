                                     ...
                                     double timestamp)
{
  ...
  std::cout << "Plane type  :" << counter.transpose() << "; Plane number: " << surfel_planes_.size() << std::endl;
...
void SurfelAssociation::getAssociation(const VPointCloud::Ptr& scan_inM, const TPointCloud::Ptr& scan_raw,
...
#pragma omp parallel for num_threads(omp_get_max_threads())
  ...
  for (int w = 0; w < width; w++)
  {
    for (int h = 0; h < height; h++)
    {
      if (associatedFlag[h * width + w] == -1 || 0 == scan_raw->at(w, h).timestamp)
        continue;
      spoint.point = Eigen::Vector3d(scan_raw->at(w, h).x, scan_raw->at(w, h).y, scan_raw->at(w, h).z);
      spoint.point_in_map = Eigen::Vector3d(scan_inM->at(w, h).x, scan_inM->at(w, h).y, scan_inM->at(w, h).z);
...
int SurfelAssociation::checkPlaneType(const Eigen::Vector3d& eigen_value, const Eigen::Matrix3d& eigen_vector,
...
bool SurfelAssociation::fitPlane(const VPointCloud::Ptr& cloud, Eigen::Vector4d& coeffs, VPointCloud::Ptr cloud_inliers)
{
...
double SurfelAssociation::point2PlaneDistance(Eigen::Vector3d& pt, Eigen::Vector4d& plane_coeff)
{
...
void SurfelAssociation::associateScanToSurfel(const size_t& surfel_idx, const VPointCloud::Ptr& scan,
                                              const double& radius, std::vector<std::vector<int>>& ring_masks) const
{
  ...
  for (int j = 0; j < scan->height; j++)
  {
    ...
    {
      if (!pcl_isnan(scan->at(i, j).x) && scan->at(i, j).x > box_min[0] && scan->at(i, j).x < box_max[0] &&
          scan->at(i, j).y > box_min[1] && scan->at(i, j).y < box_max[1] && scan->at(i, j).z > box_min[2] &&
